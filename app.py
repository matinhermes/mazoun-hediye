...
def checkout_confirm():
    ...
    # Stock will be decremented after payment confirmation
...
# /admin/add-category removed - use /admin/categories/add instead
    ...
    if 'image' in request.files:
        ...
        if file.filename:
            ...
            if ext not in allowed:
                flash('فقط فایل‌های تصویری مجاز هستند (png, jpg, gif, webp)', 'danger')
                return redirect(url_for('admin_banners'))
            import base64 as b64
            data = file.read()
            if len(data) > 5 * 1024 * 1024:  # 5MB max
                flash('حجم فایل نباید بیشتر از ۵ مگابایت باشد', 'danger')
                return redirect(url_for('admin_banners'))
...
@admin_required
def fix_images():
    """Fix products that have gradient images - only replaces gradient/placeholder images"""
    db = get_db()
    
    # Map product IDs to correct image paths - ONLY for gradient images
    image_map = {
        1: '/static/images/products/1_shomiz.jpg',
        2: '/static/images/products/2_manto.jpg',
        3: '/static/images/products/3_set.jpg',
...
def fix_all():
    ...
    db.commit()
    flash('تصاویر محصولات بروزرسانی شد ✓', 'success')
    return redirect(url_for('admin_dashboard'))
